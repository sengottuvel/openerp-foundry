...
class kg_chemical_master(osv.osv):
	...
	def _name_validate(self, cr, uid,ids, context=None):
		...
		if rec.name:
			...
			cr.execute(""" select upper(name) from kg_chemical_master where upper(name)  = '%s' """ %(name))
			data = cr.dictfetchall()			
			if len(data) > 1:
				res = False
			else:
	...
	def _code_validate(self, cr, uid,ids, context=None):
		...
		if rec.code:
			...
			cr.execute(""" select upper(code) from kg_chemical_master where upper(code)  = '%s' """ %(code))
			data = cr.dictfetchall()			
			if len(data) > 1:
				res = False
			else:
	...
	def write(self, cr, uid, ids, vals, context=None):
		...
		return super(kg_chemical_master, self).write(cr, uid, ids, vals, context)
