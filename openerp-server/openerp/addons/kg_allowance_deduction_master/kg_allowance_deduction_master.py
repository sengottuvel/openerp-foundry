...
class kg_allowance_deduction_master(osv.osv):
	...
	def write(self, cr, uid, ids, vals, context=None):
		...
		return super(kg_allowance_deduction_master, self).write(cr, uid, ids, vals, context)	
	...
	def val_negative(self,cr,uid,ids,context=None):
		...
		if rec.sequence < 0:
			raise osv.except_osv(_('Warning!'),
						_('Negative Values are not allowed for Sequence !!'))
		
		if rec.sequence:
				add_ids = self.pool.get('hr.salary.rule').search(cr,uid,[('sequence','=',rec.sequence)])
				print "----------------------------------------------",add_ids
				print "----------------------------------------------",rec.sequence
				if len(add_ids) > 1:
					raise osv.except_osv(_('Warning!'),
						_('This Sequence is assigned to other Allowance !!'))
					return False
	...
	def send_to_dms(self,cr,uid,ids,context=None):
		...
		url = 'http://192.168.1.7/sam-dms/login.html?xmxyypzr='+encoded_user+'&mxxrqx='+encoded_pwd+'&allowance_deduction='+rec_code
