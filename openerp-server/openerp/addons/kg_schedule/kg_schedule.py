				...
				if allocation_qty:
					if allocation_qty[0] != None:
						...
						cr.execute(""" select sum(available_qty) as stock_qty
							from ch_stock_inward_details  
							where pattern_id = %s and moc_id = %s
							and foundry_stock_state = 'ready_for_ms' and available_qty > 0  """%(schedule_item.pattern_id.id,schedule_item.moc_id.id))
						...
						if stock_inward_qty:
							if stock_inward_qty[0] != None:
								...
								cr.execute(""" select id,available_qty
									...
									and available_qty > 0 """%(schedule_item.pattern_id.id,schedule_item.moc_id.id))
						...
						cr.execute(""" select sum(available_qty) as stock_qty
							from ch_stock_inward_details  
							where pattern_id = %s and moc_id = %s
							and foundry_stock_state = 'foundry_inprogress' and available_qty > 0  """%(schedule_item.pattern_id.id,schedule_item.moc_id.id))
						stock_inward_qty = cr.fetchone();
