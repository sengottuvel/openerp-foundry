...
class kg_qc_verification(osv.osv):
	...
	def qc_process(self,cr,uid,ids,reject_qty,context=None):
		...
		if reject_qty > 0:
			...
			if entry.stock_type == 'pattern':
				...
				cr.execute(""" select id,available_qty as stock_qty,stock_location_id 
					...
					and id != %s """%(entry.pattern_id.id,entry.moc_id.id,entry.stock_inward_id.id))
			...
			if entry.stock_type == 'pump':
				...
				cr.execute(""" select id,available_qty as stock_qty,serial_no,stock_location_id 
					...
					and id != %s 
					order by serial_no """%(entry.pump_model_id.id,entry.stock_inward_id.id))
				stock_inward_items = cr.dictfetchall();
				...
				if stock_inward_items:
					...
					pump_rem_qty = entry.order_line_id.qty - reject_qty
	...
	def entry_confirm(self,cr,uid,ids,context=None):
		...
		if entry.state == 'draft':
			...
			self.qc_process(cr,uid,ids,entry.reject_qty)
			...
			if scrap_qty > 0:
				...
				self.qc_process(cr,uid,ids,scrap_qty)
				
				print "sssssss",scrap_qty,entry.stock_inward_id.available_qty
				if entry.stock_inward_id:
					inward_line_obj = self.pool.get('ch.stock.inward.details')
					inward_line_obj.write(cr, uid, [entry.stock_inward_id.id],{'available_qty': entry.stock_inward_id.available_qty + scrap_qty})
				else:
					if entry.stock_type == 'pump': 
