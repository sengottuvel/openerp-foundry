...
class kg_excel_attendance_report(osv.osv):

	_name = 'kg.excel.attendance.report'
	...
	def produce_xls(self, cr, uid, ids, context={}):
		...
		sql = """		
			...
			(select count(id) from ch_daily_attendance where employee_id=leaves.employee_id 
			and date >="""+date_from+""" and date <="""+date_to+""" and status in ('present','holiday','weekoff')) as worked_days,
			(select count(id) from ch_daily_attendance where employee_id=leaves.employee_id 
			and date >="""+date_from+""" and date <="""+date_to+""" and status in ('absent')) as lop,
			(select count(id) from hr_holidays where from_date>="""+date_from+""" and to_date<="""+date_to+""" and holiday_status_id=24
			and status='approved' and employee_id=leaves.employee_id) as el_days,
			(select count(id) from hr_holidays where from_date>="""+date_from+""" and to_date<="""+date_to+""" and holiday_status_id=23
			and status='approved' and employee_id=leaves.employee_id) as cl_days,
			(select count(id) from hr_holidays where from_date>="""+date_from+""" and to_date<="""+date_to+""" and holiday_status_id=27
			and status='approved' and employee_id=leaves.employee_id) as esi_days
			...
			left join kg_daily_attendance daily_att on (daily_att.id=emp.dep_id)
		...
		sheet1 = wbk.add_sheet('ATTENDANCE_REPORT')
		s2=6
		...
		sheet1.write_merge(4, 4, 0, 10,"ATTENDANCE REPORT FOR THE PERIOD "+ date_from + " "+ "TO "+date_to,style1)
		...
		sheet1.write(s2,6,"CL",style1)
		sheet1.write(s2,7,"EL",style1)
		sheet1.write(s2,8,"LOP",style1)
		sheet1.write(s2,9,"ABS",style1)
		sheet1.write(s2,10,"ESI",style1)
		...
		for ele in data:
			...
			sheet1.write(s2,5,ele['worked_days'],style1)
			sheet1.write(s2,6,ele['cl_days'],style1)
			sheet1.write(s2,7,ele['el_days'],style1)
			sheet1.write(s2,8,ele['lop'],style1)
			sheet1.write(s2,9,ele['lop'],style1)
			sheet1.write(s2,10,ele['esi_days'],style1)
