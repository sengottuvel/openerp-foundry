...
class kg_crm_enquiry(osv.osv):
	...
	_columns = {
		...
		'division_id': fields.many2one('kg.division.master','Division',readonly=True,states={'draft':[('readonly',False)]},domain="[('state','=','approved'),('code','in',('CPD','IPD'))]"),
		...
		'customer_id': fields.many2one('res.partner','Customer Name',domain=[('customer','=',True),('contact','=',False),('partner_state','=','approve')],readonly=True, states={'draft':[('readonly',False)]}),
		'dealer_id': fields.many2one('res.partner','Dealer Name',domain=[('dealer','=',True),('contact','=',False),('partner_state','=','approve')]),
		'industry_id': fields.many2one('kg.industry.master','Sector',readonly=True,states={'draft':[('readonly',False)]},domain="[('state','=','approved')]"),
	...
	_defaults = {
		...
		'company_id': lambda self,cr,uid,c: self.pool.get('res.company')._company_default_get(cr, uid, 'kg.crm.enquiry', context=c),
		...
		if entry.entry_mode == 'manual':
			if entry.ch_line_ids:
				for rec in entry.ch_line_ids:
					...
					applicable_a = 'no'
					applicable_b = 'no'
					if rec.purpose_categ == 'in_development':
						cr.execute('''select 
										dev_line.is_applicable,
										dev_line.position_no,
										dev_line.pattern_no,
										dev_line.pattern_name,
										dev_line.material_code,
										dev_line.moc_id,
										dev_line.csd_no,
										dev_line.prime_cost,
										dev_line.remarks
										from ch_crm_development_details dev_line
										left join ch_kg_crm_pumpmodel line on line.id = dev_line.header_id
										where dev_line.header_id = %s
										GROUP BY is_applicable,position_no,pattern_no,pattern_name,material_code,moc_id,csd_no,prime_cost,remarks
										HAVING ( COUNT(moc_id) > 1 ) ''',[rec.id])
						dup_ids = cr.fetchall()
						if dup_ids:
							raise osv.except_osv(_('Warning !'),
								_('Pump (%s) Pattern No. (%s) Duplicate are not allowed !!')%(rec.pump_id.name,dup_ids[0][2]))
					if rec.purpose_categ == 'pump':
						if rec.line_ids_moc_a:
							cr.execute('''SELECT offer_id, moc_id
										FROM ch_moc_construction where header_id = %s
										GROUP BY offer_id,moc_id
										HAVING ( COUNT(offer_id) > 1 )''',[rec.id])
							dup_ids = cr.fetchall()
							if dup_ids:
								for item in dup_ids:
									mat_rec = self.pool.get('kg.offer.materials').browse(cr,uid,item[0])
									raise osv.except_osv(_('Warning !'),
										_('Pump (%s) Duplicate Offer Material Name (%s) are not allowed !!')%(rec.pump_id.name,mat_rec.name))
					if rec.purpose_categ == 'spare' or rec.purpose_categ == 'pump':
						...
						if rec.line_ids_b:
							for ele in rec.line_ids_b:
								...
								if ele.is_applicable == True and not ele.moc_id and ele.flag_is_bearing == True:
									raise osv.except_osv(_('Warning !'),
										_('Pump (%s) BOT (%s) Kindly configure Brand !!'%(rec.pump_id.name,ele.ms_id.name)))
							...
							if rec.purpose_categ == 'spare' and not rec.line_ids_spare_bom:
								...
								for spare_bom in rec.line_ids_spare_bom:
									...
									if not fou_data and not ms_data and not bot_data:
										raise osv.except_osv(_('Warning !'),_('Spare (%s) Spare BOM (%s) Kindly choose Is applicable in components !!'%(rec.pump_id.name,spare_bom.bom_id.name)))
	...
	def _check_lineitems(self, cr, uid, ids, context=None):
		entry = self.browse(cr,uid,ids[0])
		if entry.name:
			cr.execute(""" select upper(name) from kg_crm_enquiry where upper(name) = '%s' and state not in ('revised','moved_to_offer') """ %(entry.name.upper()))
		...
		if entry.entry_mode == 'manual':
			if not entry.ch_line_ids:
				...
				for line in entry.ch_line_ids:
					if line.load_bom != True and line.purpose_categ in ('pump'):
	...
	def entry_revision(self,cr,uid,ids,context=None):
		...
		if entry.state == 'moved_to_offer':
			...
			if entry.wo_flag == False:
				...
				if off_ids:
					for item in off_ids:
						off_rec = off_obj.browse(cr,uid,item)
						off_obj.write(cr,uid,off_rec.id,{'state':'revised'})
				...
				self.write(cr, uid, ids, {
										  'state': 'revised',
										  'revision_remarks': entry.revision_remarks,
										})
	...
	def entry_reset(self,cr,uid,ids,context=None):
		...
		if entry.state == 'revised':
			...
			draft_wo_ids = wo_obj.search(cr,uid,[('enquiry_no','=',entry.name),('state','=','draft')])
			if draft_wo_ids:
				del_sql = """ delete from kg_work_order where id = %s """ %(draft_wo_ids[0])
				cr.execute(del_sql)
			wo_ids = wo_obj.search(cr,uid,[('enquiry_no','=',entry.name),('state','!=','draft')])
			if wo_ids:
	...
	def send_to_dms(self,cr,uid,ids,context=None):
		...
		rec_number = str(rec.name)
		encoded_user = base64.b64encode(rec_user)
		encoded_pwd = base64.b64encode(rec_pwd)
		
		if rec.source == 'service':
			url = 'http://10.100.9.32/sam-dms/login.html?xmxyypzr='+encoded_user+'&mxxrqx='+encoded_pwd+'&service_enquiry='+rec_number
		else:
			url = 'http://10.100.9.32/sam-dms/login.html?xmxyypzr='+encoded_user+'&mxxrqx='+encoded_pwd+'&marketing_enquiry='+rec_number
						...
						if brandmoc_line_data:
							...
							if raw_line.uom.id == brandmoc_rec.uom_id.id:
								...
								if brandmoc_rec.uom_id.code == 'Kg':
									...
									raise osv.except_osv(_('Warning!'),_('UOM mismatched for BOT (%s) Item (%s) check with Product Master !!'%(bot_rec.name,brandmoc_rec.product_id.name)))
						else:
							qty = design_rate = price = 0
						tot_price += price
						prime_cost = tot_price
	...
	def prime_cost_update(self,cr,uid,ids,context=None):
				...
				for order_item in entry.ch_line_ids:
					...
					if order_item.purpose_categ == 'pump':					
						pump_offer_id = self.pool.get('ch.pump.offer').create(cr,uid,{'header_id': offer_id,
																					  ...
																					  'qty': order_item.qty,
																					  'invoice_pending_qty': order_item.qty,
				...
				for access in entry.ch_line_ids_e:
					...
					if access_rec.line_ids:
						for acc_bot_item in access_rec.line_ids:
							...
							prime_cost += acc_bot_item * acc_bot_item.qty
			...
			if order_item.line_ids_access_a:
				...
				for foundry_item in order_item.line_ids_access_a:
					...
					access_id = self.pool.get('ch.accessories.offer').create(cr,uid,{
																...
																'invoice_pending_qty': foundry_item.qty,
  	...
  	def spare_bom_creation(self,cr,uid,offer_id,orde_item,spare_bom_prime_cost,context=None):
		...
		spare_id = self.pool.get('ch.spare.offer').create(cr,uid,{'header_id': offer_id,
															  ...
															  'moc_changed_flag': '',
															  'qty': orde_item.qty,
															  'invoice_pending_qty': orde_item.qty,
															  'hsn_no': hsn,
															  'gst': gst,
		...
		if item_type == 'foundry':
			...
			if bom_item.header_id.header_id.segment == 'dom':
				...
				if not bom_item.pattern_id.hsn_no.igst_id:
					raise osv.except_osv(_('Warning!'), _('Configure Pump of (%s) IGST Tax in HSN master for Pattern %s and HSN %s !!' %(bom_item.header_id.pump_id.name,bom_item.pattern_id.pattern_name,bom_item.pattern_id.hsn_no.name)))
	...
	def entry_call_back(self,cr,uid,ids,context=None):
		...
		if rec.state == 'draft':
			if not rec.reminder_date:
				raise osv.except_osv(_('Warning !'),_('Reminder Date is must !!'))
...
class ch_kg_crm_pumpmodel(osv.osv):
	...
	_columns = {
		...
		'template_id':fields.many2one('ch.kg.crm.pumpmodel','Template Name',domain="[('line_state','=','approve')]"),
		...
		'fluid_id': fields.many2one('kg.fluid.master','Liquid',domain="[('state','=','approved')]"),
		...
		'pump_id': fields.many2one('kg.pumpmodel.master','Pump Model',domain="[('state','=','approved')]"),
		'alias_name': fields.char('Alias Name'),
		'item_code': fields.char('Item Code'),
		'sump_depth': fields.char('Sump Depth'),
		'spare_pump_id': fields.many2one('kg.pumpmodel.master','Pump Model',domain="[('state','=','approved')]"),
		...
		'bearing_make': fields.many2one('kg.brand.master','Bearing Make',domain="[('state','=','approved')]"),
		'engine_make': fields.many2one('kg.brand.master','Engine Make',domain="[('state','=','approved')]"),
		'motor_make': fields.many2one('kg.brand.master','Motor Make',domain="[('state','=','approved')]"),
		...
		'shaft_sealing': fields.selection([('gld_packing_tiga','Gland Packing-TIGA'),('gld_packing_ptfe','Gland Packing-PTFE'),('gld_packing_tiwa','Gland Packing-TIWA'),('gld_packing_tiba','Gland Packing-TIBA'),
						('mc_seal','M/C Seal'),('f_s','Felt Seal'),('dynamic_seal','Dynamic Seal'),('exp_mc_seal','Expeller with MC Seal')],'Shaft Sealing'),
		'scope_of_supply': fields.selection([('bare_pump','Bare Pump'),('pump_with_acces','Pump With Accessories'),('pump_with_acces_motor','Pump With Accessories And Motor')],'Scope of Supply'),
		'drive': fields.selection([('motor','MOTOR'),('vfd','VFD'),('engine','ENGINE')],'Drive'),
		...
		'qap_plan_id': fields.many2one('kg.qap.plan', 'QAP Standard',domain="[('state','=','approved')]"),
		'spare_qap_plan_id': fields.many2one('kg.qap.plan', 'QAP Standard',domain="[('state','=','approved')]"),
		...
		'spare_pumpseries_id': fields.many2one('kg.pumpseries.master','Pump Series',domain="[('state','=','approved')]"),
		'primemover_id': fields.many2one('kg.primemover.master','Prime Mover',domain="[('state','=','approved')]"),
		...
		'spare_moc_const_id':fields.many2one('kg.moc.construction', 'MOC Construction',domain="[('state','=','approved')]"),
		'mech_seal_make':fields.many2one('kg.brand.master', 'Mech. Seal Make',domain="[('state','=','approved')]"),
		...
		'coupling_make': fields.many2one('kg.brand.master','Coupling Make',domain="[('state','=','approved')]"),
		'coupling_type_id': fields.many2one('kg.coupling.type','Coupling Type',domain="[('state','=','approved')]"),
	...
	def _line_validation(self, cr, uid, ids, context=None):
		rec = self.browse(cr, uid, ids[0])
		if rec.purpose_categ == 'pump':
			if rec.qty <= 0.00:
				raise osv.except_osv(_('Warning!'),_('Qty should be greater than Zero for Pump (%s)'%(rec.pump_id.name)))
		if rec.acces == 'yes' and not rec.line_ids_access_a:
			if rec.purpose_categ in ('pump','spare','pump_spare','access'):
				raise osv.except_osv(_('Warning!'),_('You cannot save without accessories for Pump (%s)'%(rec.pump_id.name)))
	...
	def onchange_load_bom(self, cr, uid, ids, load_bom,pump_id,wo_line_id,purpose_categ,flag_standard,moc_const_id,qty,
		motor_power,del_pipe_size,shaft_sealing,setting_height,motor_kw,bush_bearing_lubrication,push_bearing,speed_in_rpm,rpm,is_selectable_all,pump_model_type):
		...
		if not moc_const_id and purpose_categ == 'pump' and load_bom == True:
			raise osv.except_osv(_('Warning!'),_('System will not accept without MOC Construction'))
		if load_bom == True:
			if not pump_model_id:
				raise osv.except_osv(_('Warning!'),_('Pump is must !!'))
			if not pump_model_type:
				raise osv.except_osv(_('Warning!'),_('Pump Type is must !!'))
			if qty <= 0:
				raise osv.except_osv(_('Warning!'),_('Qty should be greater than Zero !!'))
			if purpose_categ == 'pump':
			...
			if rpm != False:
				...
				if shaft_sealing != False and motor_power != False and bush_bearing != False and setting_height > 0 and delivery_pipe_size != False and lubrication != False:
					...
					cr.execute(''' 
								...
								-- Bed Assembly ----
								select id,pos_no,position_id,ms_id,csd_no,name,qty,header_id as bom_id
					...
					cr.execute(''' 
								...
								-- Bed Assembly ----
								select id,bot_id,csd_no,qty,position_id,header_id as bom_id
								from ch_bot_details
								where header_id =
								(
								select id from kg_bom 
	...
	def onchange_pumpmodel(self, cr, uid, ids, pump_id, division_id,suction_pressure_kg,discharge_pressure_kg,purpose_categ, context=None):
		value = {'impeller_type': '','impeller_number': '','impeller_dia_max': '','impeller_dia_min': '','maximum_allowable_soild': '',
				...
				'sealing_water_pressure':'','lubrication_type':'','del_pipe_size':'','alias_name':''}
		total = 0.00
		...
		if pump_id:
			...
			'sealing_water_pressure':total,'lubrication_type':pump_rec.lubrication_type,'del_pipe_size':pump_rec.delivery_pipe_size,'alias_name':pump_rec.name}
		return {'value': value}
...
class ch_kg_crm_foundry_item(osv.osv):
	...
	def onchange_pattern_name(self, cr, uid, ids, pattern_id):
		...
		if pattern_obj:
			...
			value = {'pattern_name':pattern_rec.pattern_name,'off_name':pattern_rec.pattern_name}
		return {'value': value}
...
class ch_kg_crm_machineshop_item(osv.osv):
	...
	_columns = {
		...
		'position_id': fields.many2one('kg.position.number','Position No.',domain="[('state','=','approved')]"),
		'csd_no': fields.char('CSD No.',readonly=True),
		'bom_id': fields.many2one('kg.bom','BOM',domain="[('state','=','approved')]"),
	...
	def onchange_ms_id(self,cr,uid,ids,ms_id, context=None):
		value = {'name':'','off_name':''}
		if ms_id:
			ms_rec = self.pool.get('kg.machine.shop').browse(cr,uid,ms_id)
			value = {'name':ms_rec.name,'off_name':ms_rec.name}
		return {'value': value}
...
class ch_kg_crm_ms_raw(osv.osv):
	...
	_columns = {
		...
		'product_id': fields.many2one('product.product','Raw Material', required=True, domain="[('product_type','in',['ms','bot','consu','coupling']),('state','=','approved')]"),			
		'uom':fields.many2one('product.uom','UOM',size=128 ,required=True,domain="[('dummy_state','=','approved')]"),
		'od': fields.float('OD'),
		'length': fields.float('Length'),
...
class ch_kg_crm_bot(osv.osv):
	...
	def onchange_ms_id(self,cr,uid,ids,ms_id, context=None):
		value = {'item_name':'','off_name':''}
		if ms_id:
			ms_rec = self.pool.get('kg.machine.shop').browse(cr,uid,ms_id)
			value = {'item_name':ms_rec.name,'off_name':ms_rec.name}
		return {'value': value}
...
class ch_kg_crm_spare_bom(osv.osv):
	...
	_columns = {
		...
		'moc_const_id':fields.many2one('kg.moc.construction','MOC Construction',domain="[('state','=','approved')]"),
		'bom_id':fields.many2one('kg.bom','BOM Name',domain="[('pump_model_id','=',parent.pump_id),('category_type','=','part_list_bom'),('state','=','approved')]"),
...
class ch_kg_crm_spare_ms(osv.osv):
	...
	_description = "Machine Shop Item Details"
...
class ch_moc_construction(osv.osv):
	...
	_columns = {
		...
		'moc_id': fields.many2one('kg.moc.master', 'MOC Name',domain="[('state','=','approved')]",required=True),
		'offer_id': fields.many2one('kg.offer.materials', 'Material Name',domain = "[('state','=','approved')]",required=True),
		'flag_standard': fields.boolean('Non Standard'),
...
class ch_kg_crm_accessories(osv.osv):
	...
	_columns = {
		...
		'access_categ_id': fields.many2one('kg.accessories.category','Accessories Categ',domain="[('state','=','approved')]"),
		'access_id': fields.many2one('kg.accessories.master','Accessories',domain="[('state','=','approved'),('access_cate_id','=',access_categ_id)]"),
		'moc_id': fields.many2one('kg.moc.master','MOC',domain="[('state','=','approved')]"),
...
class ch_crm_component_bot(osv.osv):
	...
	_columns = {
		...
		'product_temp_id':fields.many2one('product.product','Product Name',domain = [('type','=','bot'),('state','=','approved')]),
		'position_id': fields.many2one('kg.position.number','Position No.',domain="[('state','=','approved')]"),
		'bom_id': fields.many2one('kg.bom','BOM',domain="[('state','=','approved')]"),
		'ms_id':fields.many2one('kg.machine.shop', 'Item Code', domain=[('type','=','bot'),('state','=','approved')], ondelete='cascade',required=True),
		'item_name': fields.related('ms_id','name', type='char', size=128, string='Item Name', store=True, readonly=True),
...
class ch_crm_component_primemover(osv.osv):
	...
	_columns = {
		...
		'primemover_id': fields.many2one('kg.primemover.master','Primemover',domain="[('state','=','approved')]"),
		'moc_id': fields.many2one('kg.moc.master','MOC',domain="[('state','=','approved')]"),
		'brand_id': fields.many2one('kg.brand.master','Brand',domain="[('state','=','approved')]"),
		'qty': fields.float('Qty'),
		'prime_cost': fields.float('Prime Cost'),
