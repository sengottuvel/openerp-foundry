...
class hr_holidays(osv.osv):
    ...
    _columns = {
        ...
        'holiday_status_id': fields.many2one("hr.holidays.status", "Leave Type",domain=[('state','=','approved')], required=True,readonly=True, states={'draft':[('readonly',False)], 'confirm':[('readonly',False)]}),
        'employee_id': fields.many2one('hr.employee', "Employee",domain=[('status','=','approved')], select=True, invisible=False, readonly=True, states={'draft':[('readonly',False)], 'confirm':[('readonly',False)]}),
