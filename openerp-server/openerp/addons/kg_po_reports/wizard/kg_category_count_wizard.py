...
class kg_category_count_wizard(osv.osv_memory):
	...
	_columns = {
		...
		'user_name': fields.many2one('res.users','User'),
		'status': fields.selection([('approved', 'Approved'),('cancelled','Cancelled')], "Status",invisible=False),
	...
	def _print_report(self, cr, uid, ids, data, context=None):
		...
		if data['form']:
			...
			return {'type': 'ir.actions.report.xml', 'report_name': 'kg.category.count.report', 'datas': data,  'name': 'Category Count Report'}
	
	
		

kg_category_count_wizard()
