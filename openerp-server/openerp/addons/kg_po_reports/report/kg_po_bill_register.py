...
class kg_po_bill_register(report_sxw.rml_parse):
	
	_name = 'kg.po.bill.register'
	

	def __init__(self, cr, uid, name, context=None):
		if context is None:
			context = {}
		super(kg_po_bill_register, self).__init__(cr, uid, name, context=context)
		self.query = ""
		self.period_sql = ""
	...
	def get_data(self,form):
		...
		self.cr.execute('''
			  ...
			  where pi.state=%s and pi.invoice_date >=%s and pi.invoice_date <=%s '''+ product + partner + '''
			  ...
			  left join res_city city on(city.id=res.city)
			  left join res_country_state state on(state.id=res.state_id)
			  JOIN kg_gengrn_purchase_invoice_line pil ON (pil.invoice_header_id=pi.id)
			  JOIN product_product prd ON (prd.id=pil.product_id)
			  JOIN product_template pt ON (pt.id=prd.product_tmpl_id)
			  JOIN product_uom uom ON (uom.id=pil.uom_id)
			  where pi.state=%s and pi.invoice_date >=%s and pi.invoice_date <=%s '''+ product + partner + '''
			  ''',(po_state,form['date_from'],form['date_to'],po_state,form['date_from'],form['date_to']))
...
report_sxw.report_sxw('report.kg.po.bill.register', 'purchase.order', 
			'addons/kg_po_reports/report/kg_po_bill_register.rml', 
			parser=kg_po_bill_register, header = False)
