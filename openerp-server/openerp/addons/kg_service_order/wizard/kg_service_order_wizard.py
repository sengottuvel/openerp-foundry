...
class kg_service_order_wizard(osv.osv_memory):
		
	_name = 'kg.service.order.wizard'
	_columns = {
		
		'order':fields.many2many('kg.service.order','kg_so_report_order','order_id','name','SO Order'),
		'supplier':fields.many2many('res.partner','kg_so_report_supplier','order_id','supplier_id','Supplier'),
	...
	def _print_report(self, cr, uid, ids, data, context=None):
		...
		if data['form']:
			...
			return {'type': 'ir.actions.report.xml', 'report_name': 'kg.service.order.report', 'datas': data}
	
	
		

kg_service_order_wizard()
