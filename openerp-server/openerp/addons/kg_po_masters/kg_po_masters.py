...
class kg_payment_master(osv.osv):
	...
	_columns = {
		...
		'code': fields.char('Code', size=128, readonly=False,states={'approved':[('readonly',True)],'reject':[('readonly',True)]}),
	...
	_defaults = {
		...
		'company_id': lambda self,cr,uid,c: self.pool.get('res.company')._company_default_get(cr, uid, 'kg.payment.master', context=c),
		'active': True,
		'state': 'draft',
	...
	def _name_validate(self, cr, uid,ids, context=None):
		...
		if rec.name:
			pay_name = rec.name
			name=pay_name.upper()			
			cr.execute(""" select upper(name) from kg_payment_master where upper(name) = '%s' """ %(name))
			data = cr.dictfetchall()
			if len(data) > 1:
				res = False
			else:
...
class kg_delivery_master(osv.osv):
	...
	_defaults = {
		...
		'creation_date': lambda * a: time.strftime('%Y-%m-%d %H:%M:%S'),
		'company_id': lambda self,cr,uid,c: self.pool.get('res.company')._company_default_get(cr, uid, 'kg.delivery.master', context=c),
		'active': True,
		'state': 'draft',
		'user_id': lambda obj, cr, uid, context: uid,
		'modify': 'no',
...
class kg_brand_master(osv.osv):
	...
	_defaults = {
		...
		'creation_date': lambda * a: time.strftime('%Y-%m-%d %H:%M:%S'),
		'company_id': lambda self,cr,uid,c: self.pool.get('res.company')._company_default_get(cr, uid, 'kg.brand.master', context=c),
		'active': True,
		'state': 'draft',
		'user_id': lambda obj, cr, uid, context: uid,
		'modify': 'no',
