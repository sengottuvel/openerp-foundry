...
### Packing SC Invoice Start

class kg_sc_packing_invoice(osv.osv):

	_name = "kg.sc.packing.invoice"
	_description = "SC Packing Invoice"
	_order = "entry_date desc"
	...
	_columns = {
		...
		'phone': fields.char('Phone',size=64),
		'contact_person': fields.char('Contact Person', size=128),				
		'inward_packing_line_ids': fields.many2many('ch.packing.inward.line','m2m_packing_inward_details' , 'inward_id', 'packing_inward_id', 'Packing Inward Items',
		...
		'invoice_copy':fields.binary('Contractor Invoice Copy'),
		'filename':fields.char('File Name'),
		'tax_id': fields.many2many('account.tax', 'subcontract_sc_packing_invoice_taxes', 'invoice_id', 'tax_id', 'Taxes'),
		'discount': fields.float('Discount Amount'),	
		'discount_per': fields.float('Discount(%)'),
		'discount_flag': fields.boolean('Discount Flag'),
		'discount_per_flag': fields.boolean('Discount Amount Flag'),
		...
		'line_ids': fields.one2many('ch.sc.packing.invoice.line', 'header_id', "Line Details"),
		'line_ids_a': fields.one2many('ch.sc.packing.invoice.expense.track','header_id',"Expense Track"),
	...
	_defaults = {
		...
		'company_id': lambda self,cr,uid,c: self.pool.get('res.company')._company_default_get(cr, uid, 'kg_sc_packing_invoice', context=c),			
		'entry_date' : lambda * a: time.strftime('%Y-%m-%d'),
		'user_id': lambda obj, cr, uid, context: uid,
		'crt_date':lambda * a: time.strftime('%Y-%m-%d %H:%M:%S'),
		'state': 'draft',		
	...
	def update_line_items(self,cr,uid,ids,context=None):
		entry = self.browse(cr,uid,ids[0])
		invoice_line_obj = self.pool.get('ch.sc.packing.invoice.line')
			
		del_sql = """ delete from ch_sc_packing_invoice_line where header_id=%s """ %(ids[0])
		cr.execute(del_sql)
		
		for item in entry.inward_packing_line_ids:			
			
			
			vals = {	
			
				'header_id': entry.id,
				'sub_dc_id':item.sub_dc_id.id,
				'sub_dc_line_id':item.sub_dc_line_id.id,
	...
	def entry_confirm(self,cr,uid,ids,context=None):
		...
		if len(entry.line_ids) == 0:		
			raise osv.except_osv(_('Invoice details is must !!'),
				_('Enter the proceed button!!'))
		if entry.state == 'draft':
			sc_inward_line_obj = self.pool.get('ch.packing.inward.line')
			...
			for line_item in entry.line_ids:								
				...
				if line_item.sub_inward_line_id.pending_qty < line_item.qty:
					raise osv.except_osv(_('Warning!'),
						_('System not allow to invoice Qty Exceed !!'))			
					
				
										
				sc_inward_line_obj.write(cr, uid, line_item.sub_inward_line_id.id, 
							{'pending_qty':line_item.sub_inward_line_id.pending_qty - line_item.qty})				
				
				
			### Sequence Number Generation  ###									
			if entry.name == '' or entry.name == False:
				sc_invoice_name = ''	
				sc_invoice_seq_id = self.pool.get('ir.sequence').search(cr,uid,[('code','=','kg.sc.packing.invoice')])
	...
	def entry_accept(self,cr,uid,ids,context=None):
		...
		if rec.state == 'approved':	
			...
			tax_sql = """ select sub_query.acc_col_id,sum(sub_query.debit) as debit
							...
							sum((line_exp.amount * ac_tax.amount)) as debit
							from 
							sc_packing_invoice_expense_taxe line_tax 
							left join ch_sc_packing_invoice_expense_track line_exp on(line_exp.id=line_tax.invoice_id)
							left join account_tax ac_tax on(ac_tax.id=line_tax.tax_id)
							left join kg_sc_packing_invoice inv on(inv.id=line_exp.header_id)
							...
							sum(((inv.amount_untaxed - inv.total_discount) * ac_tax.amount)) as debit
							from 
							subcontract_sc_packing_invoice_taxes line_tax 

							left join kg_sc_packing_invoice inv on(inv.id=line_tax.invoice_id)
							left join account_tax ac_tax on(ac_tax.id=line_tax.tax_id)			
	...
	def create(self, cr, uid, vals, context=None):
		return super(kg_sc_packing_invoice, self).create(cr, uid, vals, context=context)
		
	def write(self, cr, uid, ids, vals, context=None):
		vals.update({'update_date': time.strftime('%Y-%m-%d %H:%M:%S'),'update_user_id':uid})
		return super(kg_sc_packing_invoice, self).write(cr, uid, ids, vals, context)
...
kg_sc_packing_invoice()


class ch_sc_packing_invoice_line(osv.osv):

	_name = "ch.sc.packing.invoice.line"
	_description = "SC Packing Invoice Line"
	...
	def _get_oper_value(self, cr, uid, ids, field_name, arg, context=None):
		...
		for entry in self.browse(cr, uid, ids, context=context):			
			...
			print"line.op_rate",entry.value 
			total_value= entry.qty * entry.value 				
	...
	_columns = {
		...
		'header_id':fields.many2one('kg.sc.packing.invoice', 'Transaction', required=1, ondelete='cascade'),
		'remark': fields.text('Remarks'),
		'active': fields.boolean('Active'),			
		...
		'sub_inward_id': fields.many2one('kg.packing.inward','Inward Id'),
		'sub_inward_line_id': fields.many2one('ch.packing.inward.line','Inward Line Id'),
		'sub_wo_line_id': fields.many2one('ch.packing.wo.line','SUB Work Line Id'),
		'sub_wo_id': fields.many2one('kg.packing.workorder','SC WO No.'),
		...
		'inward_qty': fields.integer('Actual Qty',readonly=True),
...
ch_sc_packing_invoice_line()


class ch_sc_packing_invoice_expense_track(osv.osv):

	_name = "ch.sc.packing.invoice.expense.track"
	_description = "Expense track"	
	...
	_columns = {
		...
		'header_id': fields.many2one('kg.sc.packing.invoice', 'Expense Track'),
		'name': fields.char('Number', size=128, select=True,readonly=False),
		'date': fields.date('Creation Date'),
		'amount': fields.float('Amount'),		
		'tax_id': fields.many2many('account.tax', 'sc_packing_invoice_expense_taxe', 'invoice_id', 'tax_id', 'Tax'),
		'company_id': fields.many2one('res.company', 'Company Name'),
		'description': fields.char('Description'),
		'remark': fields.text('Remarks'),		
		'expense_amt': fields.function(_get_total_amt, string='Total Amount',digits=(16,2), method=True, store=True, type='float'),		
	...
	_defaults = {
		...
		'company_id': lambda self,cr,uid,c: self.pool.get('res.company')._company_default_get(cr, uid, 'ch.sc.packing.invoice.expense.track', context=c),
		'date' : lambda * a: time.strftime('%Y-%m-%d'),
	
		}	
	
ch_sc_packing_invoice_expense_track()
