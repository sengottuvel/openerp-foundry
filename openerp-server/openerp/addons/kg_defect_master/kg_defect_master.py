...
class kg_defect_master(osv.osv):

	_name = "kg.defect.master"
	...
	_columns = {
		...
		'is_full_pump': fields.boolean('Is Full Pump Defect'),
	...
	_defaults = {
		...
		'company_id': lambda self,cr,uid,c: self.pool.get('res.company')._company_default_get(cr, uid, 'kg.defect.master', context=c),
		'active': True,		
		'state': 'draft',
	...
	def _name_validate(self, cr, uid,ids, context=None):
		...
		if rec.name:
			...
			cr.execute(""" select upper(name) from kg_defect_master where upper(name)  = '%s' """ %(name))
			data = cr.dictfetchall()			
			if len(data) > 1:
				res = False
			else:
	...
	def _code_validate(self, cr, uid,ids, context=None):
		...
		res = True
		if rec.code:
			chemical_code = rec.code
			code=chemical_code.upper()			
			cr.execute(""" select upper(code) from kg_defect_master where upper(code)  = '%s' """ %(code))
			data = cr.dictfetchall()			
			if len(data) > 1:
				res = False
			else:
	...
	def write(self, cr, uid, ids, vals, context=None):
		...
		return super(kg_defect_master, self).write(cr, uid, ids, vals, context)

	_constraints = [
		
		(_Validation, 'Special Character Not Allowed!', ['Check Name']),
		(_CodeValidation, 'Special Character Not Allowed!!', ['Check Code']),
		(_name_validate, 'Defect name must be unique!', ['name']),
		(_code_validate, 'Defect code must be unique!', ['code']),
