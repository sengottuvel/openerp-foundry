...
class gate_pass_register_wiz(osv.osv_memory):
	...
	_columns = {
		...
		'dep_id':fields.many2many('kg.depmaster','gate_pass_reg','wiz_id','dep_id','Department Name'),
		...
		'product':fields.many2many('product.product','gate_pass_reg_product','product_wiz_id','product_dep_id','Product'),
	...
	_defaults = {
		...
		'company_id': lambda self,cr,uid,c: self.pool.get('res.company')._company_default_get(cr, uid, 'gate.pass.register.wiz', context=c),
	}
