...
class dep_issue_register_wizard(osv.osv_memory):
	...
	_columns = {
		...
		'dep_id':fields.many2many('kg.depmaster','dep_issue_register','wiz_id','dep_id','Department Name'),
		...
		'product':fields.many2many('product.product','dep_issue_product','product_wiz_id','product_dep_id','Product'),
	...
	_defaults = {
		...
		'company_id': lambda self,cr,uid,c: self.pool.get('res.company')._company_default_get(cr, uid, 'dep.issue.register.wizard', context=c),
	}
	...
	def _print_report(self, cr, uid, ids, data, context=None):
		...
		if data['form']:
			...
			return {'type': 'ir.actions.report.xml', 'report_name': 'dep.issue.register', 'datas': data,  'name': 'Department Issue Register'}	
