...
class kg_serindent_detail_wizard(osv.osv_memory):
		
	_name = 'kg.serindent.detail.wizard'
	_columns = {
		
		'dep_id':fields.many2many('kg.depmaster','kg_ser_indent_wizard','dep_id','indent_id','Dep.Name'),
	...
	_defaults = {
		...
		'company_id': lambda self,cr,uid,c: self.pool.get('res.company')._company_default_get(cr, uid, 'kg.serindent.detail.wizard', context=c),
	}
	...
	def _print_report(self, cr, uid, ids, data, context=None):
		...
		if data['form']:
			...
			return {'type': 'ir.actions.report.xml', 'report_name': 'kg.serindent.detail.report', 'datas': data,  'name': 'Service Indent Register'}
	
kg_serindent_detail_wizard()
