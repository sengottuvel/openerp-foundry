...
class kg_serindent_detail_report(report_sxw.rml_parse):
	
	_name = 'kg.serindent.detail.report'
	
	
	def __init__(self, cr, uid, name, context=None):
		if context is None:
			context = {}
		super(kg_serindent_detail_report, self).__init__(cr, uid, name, context=context)
		self.query = ""
		self.period_sql = ""
	...
	def get_data(self,form):
		...
		self.cr.execute('''
				...
				to_char(indent.date,'dd/mm/yyyy') as indent_date,
				indent.name as indent_number,
				indent.dep_name as dep_id,
				dep.dep_name as department_name,
				service_line.product_id as product_id,
				service_line.uom as uom,
				service_line.qty as indent_qty,
				service_line.pending_qty as pending_qty,
				service_line.issue_pending_qty as issue_pending_qty,
				...
				FROM  kg_service_indent indent
				
				join kg_depmaster dep  on (dep.id=indent.dep_name)
				join kg_service_indent_line service_line  on (indent.id=service_line.service_id)
				join product_product product  on (product.id=service_line.product_id)
				join product_uom uom on (uom.id=service_line.uom)
				
				
				
				where indent.date >=%s and indent.date <=%s and indent.state = %s '''+ where_sql + '''
				order by indent.date ''',(form['date_from'],form['date_to'],ind_state))
...
report_sxw.report_sxw('report.kg.serindent.detail.report','kg.service.indent','addons/kg_store_reports/report/kg_serindent_detail_report.rml',
						parser=kg_serindent_detail_report, header= False)
