...
class kg_sale_cr_note(osv.osv):

	_name = "kg.sale.cr.note"
	_description = "Sales CR Note"
	...
	def _amount_all(self, cr, uid, ids, field_name, arg, context=None):
		...
		total_amt = round_off_amt = net_amt = 0.00
		for order in self.browse(cr, uid, ids, context=context):
			...
			res[order.id]['net_amt'] = total_amt + order.round_off_amt
		return res
	
	def _get_order(self, cr, uid, ids, context=None):
		result = {}
		for line in self.pool.get('ch.sale.cr.note').browse(cr, uid, ids, context=context):
	...
	_columns = {
		...
		'name': fields.char('Credit Note No', size=128, required=True, select=True),	
		...
		'credit_note_date':fields.date('Credit Note Date'),	
		'invoice_id':fields.many2one('kg.sale.invoice','Invoice No',domain=[('state','=','invoice')]),	
		...
		'line_id':fields.one2many('ch.sale.cr.note', 'header_id', "Credit Note Line"),		
	}
	
	_defaults = {
	
		'company_id': lambda self,cr,uid,c: self.pool.get('res.company')._company_default_get(cr, uid, 'kg.sale.cr.note', context=c),
		'active': True,
		'state': 'draft',
		...
		'modify': 'no',
		'entry_mode': 'manual',
		'credit_note_date' : lambda * a: time.strftime('%Y-%m-%d'),
		
	}
	
	_sql_constraints = [
	
		('name', 'unique(name)', 'Name must be unique for a Credit Note !!'),
	]
	...
	def write(self, cr, uid, ids, vals, context=None):
		...
		return super(kg_sale_cr_note, self).write(cr, uid, ids, vals, context)
...
class ch_sale_cr_note(osv.osv):

	_name = "ch.sale.cr.note"
	...
	_columns = {
		...
		'header_id':fields.many2one('kg.sale.cr.note', 'Header_id', ondelete='cascade'),		
		...
		'discount_amt': fields.float('Discount amount(Rs)'),
		
		'remarks': fields.text('Remarks'),	
		
		
		'sub_total': fields.function(_get_total_amt, string='Grand Amount',digits=(16,2), method=True, store=True, type='float'),
