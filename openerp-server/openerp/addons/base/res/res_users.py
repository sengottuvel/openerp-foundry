...
class res_users(osv.osv):
	...
	_columns = {
		...
		'password': fields.char('Password', size=64, invisible=False,
...
class users_implied(osv.osv):
	...
	def create(self, cr, uid, values, context=None):
		...
		if groups:
			...
			for group_id in groups:
				if group_id[0] == 4:
					cr.execute(''' select menu_id from ir_ui_menu_group_rel where gid = %s ''',[group_id[1]])
					menu_ids = cr.fetchall()
					for menu_item in menu_ids:
						menu_items.append(menu_item[0])
			values.update({'user_menu_access': [(4, menu) for menu in menu_items]})
		### Ends Here ###
	...
	def write(self, cr, uid, ids, values, context=None):
		...
		if values.get('groups_id'):
			#~ ### Added by Sangeetha ###
			menu_items = []
			for group_id in values.get('groups_id'):
				if group_id[0] == 3:
					cr.execute(''' select menu_id from ir_ui_menu_group_rel where gid = %s ''',[group_id[1]])
					menu_ids = cr.fetchall()
					for menu_item in menu_ids:
						menu_items.append(menu_item[0])
					values.update({'user_menu_access': [(3, menu) for menu in menu_items]})
				if group_id[0] == 4:
					cr.execute(''' select menu_id from ir_ui_menu_group_rel where gid = %s ''',[group_id[1]])
					menu_ids = cr.fetchall()
					for menu_item in menu_ids:
