...
class kg_moc_category(osv.osv):

	_name = "kg.moc.category"
	_description = "MOC Category Master"
	...
	_defaults = {
		...
		'company_id': lambda self,cr,uid,c: self.pool.get('res.company')._company_default_get(cr, uid, 'kg.moc.category', context=c),
		'active': True,
		'state': 'draft',
	...
	def _code_validate(self, cr, uid,ids, context=None):
		...
		res = True
		if rec.code:
			moc_cate_code = rec.code
			code=moc_cate_code.upper()			
			cr.execute(""" select upper(code) from kg_moc_category where upper(code)  = '%s' """ %(code))
			data = cr.dictfetchall()			
			if len(data) > 1:
				res = False
			else:
	...
	def write(self, cr, uid, ids, vals, context=None):
		...
		return super(kg_moc_category, self).write(cr, uid, ids, vals, context)
		
	
	_constraints = [
		(_Validation, 'Special Character Not Allowed !!!', ['Check Name']),
		(_CodeValidation, 'Special Character Not Allowed !!!', ['Check Code']),
		(_name_validate, 'MOC Category name must be unique !!', ['name']),		
		(_code_validate, 'MOC Category code must be unique !!', ['code']),		
