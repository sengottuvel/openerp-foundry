...
class kg_flange_master(osv.osv):

	_name = "kg.flange.master"
	_description = "Flange Master"
	...
	_defaults = {
		...
		'company_id': lambda self,cr,uid,c: self.pool.get('res.company')._company_default_get(cr, uid, 'kg.flange.master', context=c),
		'active': True,		
		'state': 'draft',
	...
	def _name_validate(self, cr, uid,ids, context=None):
		...
		if rec.name:
			flange_name = rec.name
			name=flange_name.upper()			
			cr.execute(""" select upper(name) from kg_flange_master where upper(name) = '%s' """ %(name))
			data = cr.dictfetchall()			
			if len(data) > 1:
				raise osv.except_osv(_('Warning!'),
					_('Flange name must be unique!'))
	...
	def _code_validate(self, cr, uid,ids, context=None):
		...
		res = True
		if rec.code:
			flange_code = rec.code
			code=flange_code.upper()			
			cr.execute(""" select upper(code) from kg_flange_master where upper(code) = '%s' """ %(code))
			data = cr.dictfetchall()			
			if len(data) > 1:
				res = False
			else:
	...
	def write(self, cr, uid, ids, vals, context=None):
		...
		return super(kg_flange_master, self).write(cr, uid, ids, vals, context)
	...
	def send_to_dms(self,cr,uid,ids,context=None):
		...
		url = 'http://192.168.1.7/sam-dms/login.html?xmxyypzr='+encoded_user+'&mxxrqx='+encoded_pwd+'&flange='+rec_code
	...
	_constraints = [
		...
		(_name_validate, 'Flange name must be unique!', ['']),		
		(_code_validate, 'Flange code must be unique!', ['code']),		
