...
class kg_moulding_log(osv.osv):

	_name = "kg.moulding.log"
	_description = "Moulding Log"
	_order = "entry_date desc"
	
	_columns = {
	
		### Header Details ####
		'name': fields.char('Moulding Log No.', size=128,select=True),
	...
	_defaults = {
		...
		'company_id': lambda self,cr,uid,c: self.pool.get('res.company')._company_default_get(cr, uid, 'kg_moulding_log', context=c),
	...
	def write(self, cr, uid, ids, vals, context=None):
		...
		return super(kg_moulding_log, self).write(cr, uid, ids, vals, context)
