...
class kg_recruitment(osv.osv):
		...
		if field_name == 'modify':
			for h in self.browse(cr, uid, ids, context=None):
				...
				if h.status == 'approved':
					...
					WHERE constraint_type = 'FOREIGN KEY' and tc.table_name not in ('hr_applicant_hr_applicant_category_rel','hr_employee')
	...
	_columns = {
		...
		'category':fields.selection([('fresher','Fresher'),('experienced','Experienced')],'Category'),
		'exp_dur':fields.float('Experience Duration'),
		'exp_domain':fields.float('Domain'),
		'resume_data':fields.binary('Resume'),
	...
	def _code_dup_validate(self, cr, uid,ids, context=None):
		...
		if rec.code:
			...
			cr.execute(""" select upper(code) from kg_division_master where upper(code)  = '%s' """ %(code))
			data = cr.dictfetchall()			
			if len(data) > 1:
				res = False
			else:
	...
	def write(self, cr, uid, ids, vals, context=None):
		...
		return super(kg_recruitment, self).write(cr, uid, ids, vals, context)
		
	
	
	### Validations###
	
	def _Validation_applicant(self, cr, uid, ids, context=None):
		flds = self.browse(cr , uid , ids[0])
		name_special_char = ''.join( c for c in flds.partner_name if  c in '!@#$%^~*{}?+/=' )		
		if name_special_char:
			return False		
		return True
		
	def _Validation_exp_salary(self, cr, uid, ids, context=None):
		flds = self.browse(cr , uid , ids[0])
		if flds.salary_expected <= 0 or flds.salary_proposed <= 0:
			return False		
		return True
	...
	def send_to_dms(self,cr,uid,ids,context=None):
		...
		rec_code = str(rec.partner_name)		
		encoded_user = base64.b64encode(rec_user)
		encoded_pwd = base64.b64encode(rec_pwd)
			
		url = 'http://192.168.1.7/sam-dms/login.html?xmxyypzr='+encoded_user+'&mxxrqx='+encoded_pwd+'&resume_bank='+rec_code
	...
	_constraints = [
		...
		(_Validation_applicant, 'Special Characters are not allowed in Applicant Name !!', ['Check Applicant']),		
		(_Validation_exp_salary, 'The Expected and Proposed salary should not be zero !!', ['']),		
