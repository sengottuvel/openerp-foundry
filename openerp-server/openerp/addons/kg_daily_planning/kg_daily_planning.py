...
class kg_daily_planning(osv.osv):
	...
	_columns = {
		...
		'schedule_line_ids':fields.many2many('ch.weekly.schedule.details','m2m_weekly_schedule_details' , 'planning_id', 'schedule_id', 'Weekly Schedule Lines',
			domain="[('header_id.state','=','confirmed'),'&',('state','=','confirmed'),'&', ('planning_qty','>','0'),'&', ('transac_state','in',('in_schedule','partial','sent_for_produc')),'|',('header_id.division_id','=',division_id),('header_id.location','=',location)]"),
	...
	def update_line_items(self,cr,uid,ids,product_id,context=False):
		...
		if entry.schedule_line_ids:
			for sch_item in entry.schedule_line_ids:
				...
				cr.execute(''' select distinct stage_id from kg_foundry_stock ''')
				result_stages = cr.fetchall()

				tot_stock = 0.00
				for stage_id in result_stages:
					...
					''',[sch_item.moc_id.id,stage_id,sch_item.pump_model_id.id, sch_item.pattern_id.id, sch_item.part_name_id.id
						,sch_item.moc_id.id,stage_id, sch_item.pump_model_id.id, sch_item.pattern_id.id, sch_item.part_name_id.id])
					...
					if stock_qty > 0.00:
						...
						allocation_item_vals = {
							...
							'schedule_id': sch_item.header_id.id,
							'schedule_line_id': sch_item.id,
							'planning_qty' : sch_item.planning_qty,										
							'qty' : 0.00,											
							'stock_qty' : stock_qty,
							'stage_id':stage_id			
						}
		...
		if entry.line_ids:
			...
			for plan_item in entry.line_ids:
				...
				if plan_item.line_ids:
					for alloc_item in plan_item.line_ids:
						...
						if alloc_item.qty > 0.00: 
							...
							''',[entry.company_id.id,entry.division_id.id or None,entry.location,plan_item.pump_model_id.id, plan_item.pattern_id.id,plan_item.part_name_id.id,
							plan_item.moc_id.id,plan_item.type,alloc_item.qty,plan_item.schedule_id.id,plan_item.schedule_line_id.id,
							entry.id,plan_item.id,alloc_item.id,today,plan_item.schedule_id.entry_date,entry.entry_date,alloc_item.stage_id.id ])
				...
				if not allocation_id or allocation_id == None:
					...
					schedule_line_rec = schedule_line_obj.browse(cr, uid, plan_item.schedule_line_id.id)
					
					planned_qty = schedule_line_rec.planning_qty - plan_item.qty
					
					schedule_line_rec.write({'planning_qty': planned_qty,'transac_state':'sent_for_produc'})
	...
	def entry_cancel(self,cr,uid,ids,context=None):
		...
		for plan_item in entry.line_ids:
			...
			if plan_item.schedule_line_id.qty == plan_item.schedule_line_id.planning_qty:
				cr.execute(''' update ch_weekly_schedule_details set transac_state = 'in_schedule' where id = %s and header_id = %s ''',[plan_item.schedule_line_id.id, plan_item.schedule_id.id])
			if plan_item.schedule_line_id.planning_qty < plan_item.schedule_line_id.qty:
				cr.execute(''' update ch_weekly_schedule_details set transac_state = 'partial' where id = %s and header_id = %s ''',[plan_item.schedule_line_id.id, plan_item.schedule_id.id])
			if plan_item.schedule_line_id.planning_qty == 0.00:
				cr.execute(''' update ch_weekly_schedule_details set transac_state = 'complete' where id = %s and header_id = %s ''',[plan_item.schedule_line_id.id, plan_item.schedule_id.id])
			
			### Updation for production ####
			planning_qty = plan_item.schedule_line_id.planning_qty + plan_item.qty
			...
			alloc_qty = cr.fetchone()
			if alloc_qty[0] == None:
				if plan_item.schedule_line_id.qty == planning_qty:
					cr.execute(''' update ch_weekly_schedule_details set planning_qty = %s, transac_state = 'in_schedule' where id = %s and header_id = %s ''',[planning_qty,plan_item.schedule_line_id.id, plan_item.schedule_id.id])
				if planning_qty < plan_item.schedule_line_id.qty:
					cr.execute(''' update ch_weekly_schedule_details set planning_qty = %s, transac_state = 'partial' where id = %s and header_id = %s ''',[planning_qty,plan_item.schedule_line_id.id, plan_item.schedule_id.id])
				if plan_item.schedule_line_id.planning_qty == 0.00:
					cr.execute(''' update ch_weekly_schedule_details set planning_qty = %s, transac_state = 'complete' where id = %s and header_id = %s ''',[planning_qty,plan_item.schedule_line_id.id, plan_item.schedule_id.id])
...
class ch_daily_planning_details(osv.osv):
	...
	_columns = {
		...
		'schedule_qty': fields.related('schedule_line_id','qty', type='float', size=100, string='Schedule Qty', store=True, readonly=True),
	...
	def entry_cancel(self,cr,uid,ids,context=None):
		...
		if entry.schedule_line_id.qty == entry.schedule_line_id.planning_qty:
			cr.execute(''' update ch_weekly_schedule_details set transac_state = 'in_schedule' where id = %s and header_id = %s ''',[entry.schedule_line_id.id, entry.schedule_id.id])
		if entry.schedule_line_id.planning_qty < entry.schedule_line_id.qty:
			cr.execute(''' update ch_weekly_schedule_details set transac_state = 'partial' where id = %s and header_id = %s ''',[entry.schedule_line_id.id, entry.schedule_id.id])
		if entry.schedule_line_id.planning_qty == 0.00:
			cr.execute(''' update ch_weekly_schedule_details set transac_state = 'complete' where id = %s and header_id = %s ''',[entry.schedule_line_id.id, entry.schedule_id.id])
		### Updation for production ####
		planning_qty = entry.schedule_line_id.planning_qty + entry.qty
		...
		alloc_qty = cr.fetchone()
		if alloc_qty[0] == None:
			if entry.schedule_line_id.qty == planning_qty:
				cr.execute(''' update ch_weekly_schedule_details set planning_qty = %s, transac_state = 'in_schedule' where id = %s and header_id = %s ''',[planning_qty,entry.schedule_line_id.id, entry.schedule_id.id])
			if planning_qty < entry.schedule_line_id.qty:
				cr.execute(''' update ch_weekly_schedule_details set planning_qty = %s, transac_state = 'partial' where id = %s and header_id = %s ''',[planning_qty,entry.schedule_line_id.id, entry.schedule_id.id])
			if planning_qty == 0.00:
				cr.execute(''' update ch_weekly_schedule_details set planning_qty = %s, transac_state = 'complete' where id = %s and header_id = %s ''',[planning_qty,entry.schedule_line_id.id, entry.schedule_id.id])
	...
	def entry_reallocate(self,cr,uid,ids,context=None):
		...
		if entry.line_ids:
			for alloc_item in entry.line_ids:
				...
				if alloc_item.qty > 0.00: 
					...
					schedule_line_obj.write(cr, uid, entry.schedule_line_id.id, {'transac_state':'sent_for_qc'})
		...
		if not allocation_id or allocation_id == None:
			...
			### Planning Qty Updation in Weekly Schedule ###
			schedule_line_rec = schedule_line_obj.browse(cr, uid, entry.schedule_line_id.id)
			
			planned_qty = schedule_line_rec.planning_qty - entry.qty
